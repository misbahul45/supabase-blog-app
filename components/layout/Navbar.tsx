...
const Navbar = () => {
    ...
    const renderComponents = isAuthenticated ? (
        ...
        <div className='space-x-4 flex items-center'>
            <Link href={'/register'} className='px-4 py-2 text-sm rounded bg-cyan-700 text-slate-100 font-bold hover:bg-cyan-600 flex items-center gap-2'>
                <UserPlus />
                <span>Join Us</span>
            </Link>
    ...
    return (
        <header className={`sticky top-0 left-0 w-full h-16 backdrop-blur-lg ${scrolling ? 'translate-y-0' : '-translate-y-20'} transition-all duration-75`}>
            <div className="w-full px-10 mx-auto h-full flex justify-between items-center">
                <Link href={'/'} className='bg-gradient-to-r from-cyan-700 to-blue-600 shadow-xl shadow-blue-500/10 w-max px-4 py-2 rounded font-semibold text-white'>
